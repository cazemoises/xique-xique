          ...
          <>
            <div className="rounded-2xl border border-sand-border bg-white p-3.5">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-medium uppercase tracking-wide text-muted-2">Entregar em</span>
                <span className="text-[11.5px] font-semibold text-terracota">trocar</span>
              </div>
            ...
            {grupos.map((grupo) => (
              <div key={grupo.poloId} className="mt-3 rounded-2xl border border-sand-border bg-white p-3.5">
                <div className="mb-2.5 flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-muted-2">{grupo.poloNome}</span>
                </div>
                {grupo.bancas.map((gb) => (
                  <div key={gb.bancaId} className="mb-2">
                    <div className="flex items-center gap-2.25 border-b border-[#F3E7DA] pb-2">
                      <span className="text-[12.5px] font-bold">{gb.bancaNome}</span>
                    </div>
                    {gb.itens.map((item) => (
                      <div
                        key={`${item.produtoId}-${item.tamanho}`}
                        className="flex items-center gap-2.75 border-b border-[#F8F0E6] py-2.5"
                      >
                        <div className="flex flex-1 flex-col gap-0.75">
                          <span className="text-[12.5px] leading-snug">{item.nome}</span>
                          <span className="text-[11px] text-muted-2">Tam. {item.tamanho}</span>
                          <span className="text-[13px] font-bold">{formatPrice(item.precoUnitario)}</span>
                        </div>
                        <span className="text-xs font-semibold text-[#B0A093]">{item.quantidade}×</span>
                      </div>
                    ))}
                  </div>
                ))}
                <div className="flex justify-between pt-0.5 text-xs font-medium text-[#5C4A3D]">
            ...
            <div className="mt-3 flex flex-col gap-2.25 rounded-2xl border border-sand-border bg-white p-3.5 text-xs text-[#5C4A3D]">
              ...
              {!cupomCodigo && (
                <button
                  ...
                  className="self-start text-[11.5px] font-semibold text-terracota"
                >
                  Aplicar cupom PRIMEIRACOMPRA
                </button>
              )}
              <div className="flex justify-between border-t border-[#F3E7DA] pt-2.25 font-display text-base font-bold text-ink">
                <span>Total</span>
            ...
            <div className="mt-3 flex items-center gap-2.75 rounded-2xl border border-sand-border bg-white p-3.5">
              ...
              <div className="flex flex-1 flex-col gap-0.5">
                <span className="text-[13px] font-semibold">Pagar com Pix</span>
                <span className="text-[11px] text-muted-2">Dinheiro e cartão na entrega também</span>
              </div>
            </div>
            <div className="h-28" />
          </>
      ...
      {itens.length > 0 && (
        <div className="fixed inset-x-0 bottom-0 z-30 mx-auto w-full max-w-[480px] border-t border-sand-border bg-white px-4.5 py-3.5 pb-7.5 md:sticky md:inset-x-auto">
          <button
            ...
            className="w-full rounded-2xl bg-terracota py-4 text-center text-[15px] font-bold text-white shadow-[0_8px_18px_rgba(140,74,58,0.3)] disabled:opacity-60"
          >
            {enviando ? 'Enviando...' : `Fazer pedido · ${formatPrice(total)}`}
          </button>
          <p className="mb-0 mt-2.25 text-center text-[11px] text-muted-2">
